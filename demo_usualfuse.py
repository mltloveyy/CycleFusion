...
args.output_dir = join_path(args.output_dir, "usualfuse_" + current_time)
...
def test(model: QualityFuser, test_set: FingerPrint, epoch, write_result=False):
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader):
            ...
            total_loss_value = restore_loss_value + fuse_loss_value
...
def train(train_set: FingerPrint, test_set: FingerPrint):
    ...
    writer = SummaryWriter(join_path(args.output_dir, "summary_usualfuse"))
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        Loss_restore, Loss_fuse = [], []

        params = [{"params": model.encoder.parameters()}, {"params": model.decoder.parameters()}]
        ...
        for i, data in enumerate(trainloader):
            ...
            losses = restore_loss_value + fuse_loss_value
            losses.backward()
            ...
            logging.info(f"[Iter{i}] restore: {restore_loss_value.item():.5f} fuse: {fuse_loss_value.item():.5f}")
