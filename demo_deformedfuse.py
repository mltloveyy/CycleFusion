...
args.output_dir = join_path(args.output_dir, "deformedfuse_" + current_time)
...
def test(model: DeformedFuser, test_set: FingerPrint, epoch, write_result=False):
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader):
            ...
            restore_loss_value, ncc_loss_value, flow_loss_value, fuse_loss_value, quality_loss_value = 0, 0, 0, 0, 0
            ...
            quality_loss_value = torch.mean((q_max - q_fused)[q_max > q_fused])  # The quality after fusion should reached the maximum of the two.
            quality_loss_value = (
                quality_loss_value - args.regular_weight * (torch.mean(q_fused[q_fused > args.quality_threshold]) - args.quality_threshold).item()
            )  # It is enough when the quality reached the threshold.

            total_loss_value = restore_loss_value + ncc_loss_value + flow_loss_value + fuse_loss_value + quality_loss_value
            test_loss += total_loss_value
            logging.debug(
                f"[Test loss] restore: {restore_loss_value:.5f} ncc: {ncc_loss_value:.5f} flow: {flow_loss_value:.5f} fuse: {fuse_loss_value:.5f} quality: {quality_loss_value:.5f}"
            )
            ...
            if write_result & (epoch % args.save_interval == 0):
                ...
                fuse_cmp = torch.cat((img_tir, img_oct, img_fused, torch.zeros_like(img_fused)), dim=-1).cpu()
                quality_cmp = torch.cat((q_tir, q_oct, q_fused, torch.zeros_like(q_fused)), dim=-1).cpu()
                output = torch.cat((rec_cmp, deform_cmp, fuse_cmp, quality_cmp), dim=-2).cpu()
...
def train(train_set: FingerPrint, test_set: FingerPrint):
    ...
    writer = SummaryWriter(join_path(args.output_dir, "summary_deformedfuse"))
    ...
    for epoch in range(args.epochs):
        ...
        Loss_restore, Loss_ncc, Loss_flow, Loss_fuse, Loss_quality = [], [], [], [], []

        optimizer1 = Adam([{"params": model.encoder.parameters()}, {"params": model.decoder.parameters()}], lr=args.lr)
        params = [{"params": model.deformer.parameters()}]
        if model.use_fusion_network:
            params.append({"params": model.fuser.parameters()})
        optimizer2 = Adam(params, lr=args.lr)
        ...
        for i, data in enumerate(trainloader):
            ...
            if i % args.critic == 0:
                ...
                quality_loss_value = torch.mean((q_max - q_fused)[q_max > q_fused])  # The quality after fusion should reached the maximum of the two.
                quality_loss_value = quality_loss_value - args.regular_weight * (
                    torch.mean(q_fused[q_fused > args.quality_threshold]) - args.quality_threshold
                )  # It is enough when the quality reached the threshold.
                loss2 = ncc_loss_value + flow_loss_value + fuse_loss_value + quality_loss_value
                loss2.backward()
                ...
                logging.info(
                    f"[Iter{i}] restore: {restore_loss_value.item():.5f} ncc: {ncc_loss_value.item():.5f} flow: {flow_loss_value.item():.5f} fuse: {fuse_loss_value.item():.5f} quality: {quality_loss_value.item():.5f}"
                )
        ...
        logging.info(f"[Train loss] restore: {restore_mean:.5f} ncc: {ncc_mean:.5f} flow: {flow_mean:.5f} fuse: {fuse_mean:.5f} quality: {quality_mean:.5f}")
        train_loss = restore_mean + ncc_mean + flow_mean + fuse_mean + quality_mean
